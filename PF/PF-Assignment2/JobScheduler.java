...
public class JobScheduler {
	...
	public static int turnAroundTime(int completionTime, int arrivalTime) {
		int turnAroundTime = Math.abs(completionTime - arrivalTime);
		return turnAroundTime;
	}

	//Returns completion time of a process.
	public static int completionTime(int index, int[][] processArray) {
	...
	public static int waitingTime(int turnAroundTime, int burstTime) {
		int waitingTime = Math.abs(turnAroundTime - burstTime);
		return waitingTime;
	}

	//Returns maximum waiting time of processes
	public static int maxWaitingTime(int[] waitingTime) {
	...
	public static double averageWait(int[] waitingTime) {
		double average = 0;
		for (int i = 0; i < waitingTime.length; i++) {
	...
	public static void main(String args[]) {
		...
		for (int i = 0; i < size; i++) {
			completionTime[i] += completionTime(i, processArray);
			turnAroundTime[i] = turnAroundTime(completionTime[i],
					processArray[i][0]);
			waitingTime[i] = waitingTime(turnAroundTime[i], processArray[i][1]);
		}
		...
		System.out.println("AVERAGE WAITING TIME: " + averageWait(waitingTime));
