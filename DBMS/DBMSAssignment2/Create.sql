...
CREATE TABLE `category` (
  ...
  `Parent_CategotyID` varchar(45) NOT NULL,
...
CREATE TABLE `product_category` (
  ...
  CONSTRAINT `ProductID` FOREIGN KEY (`ProcudtID`) REFERENCES `product` (`ProductID`) ON DELETE NO ACTION ON UPDATE NO ACTION
);
...
CREATE TABLE `ordered_product`(
    `Order_ProductID` int(11) NOT NULL AUTO_INCREMENT,
    `OrderID` int(11) NOT NULL,
    `ProductID` int(11) NOT NULL,
    `Order_Price` float NOT NULL,
    PRIMARY KEY (`Order_ProductID`),
    CONSTRAINT `OrdIDOrderProd` FOREIGN KEY (`OrderID`) REFERENCES `order` (`OrderID`),
    CONSTRAINT `ShopIDOrderProd` FOREIGN KEY (`ProductID`) REFERENCES `shopper` (`ShopperID`)
);
